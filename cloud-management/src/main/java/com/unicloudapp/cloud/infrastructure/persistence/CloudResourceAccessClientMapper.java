...
interface CloudResourceAccessClientMapper {
    ...
    default CloudConnectorEntity toEntity(CloudConnector domain) {
        ...
        return CloudConnectorEntity.builder()
                .id(
                        domain.getCloudConnectorId() != null
                                ? domain.getCloudConnectorId().id()
                                : null)
                .name(domain.getName())
                .host(domain.getHost())
                .port(domain.getPort())
                .defaultCostLimit(
                        domain.getDefaultCostLimit() != null
                                ? domain.getDefaultCostLimit().getCost()
                                : null)
                .defaultCleanUpCron(
                        domain.getCronExpression() != null
                                ? domain.getCronExpression().toString()
                                : null)
                .resourceTypes(
                        domain.getResourceTypes() != null
                                ? domain.getResourceTypes().stream()
    ...
    default CloudConnector toDomain(CloudConnectorEntity entity) {
        ...
        return CloudConnector.builder()
                ...
                .port(entity.getPort())
                .defaultCostLimit(
                        entity.getDefaultCostLimit() != null ? CostLimit.of(entity.getDefaultCostLimit()) : null)
                .cronExpression(
                        entity.getDefaultCleanUpCron() != null
                                ? CronExpression.parse(entity.getDefaultCleanUpCron())
                                : null)
                .resourceTypes(
                        entity.getResourceTypes() != null
